...
# FORK: collect the fork-owned STR_* keys declared in the ForkStrId enum so the
# upstream validator can ignore them (they're stored separately, not in YAML).
def _load_fork_string_keys(fork_keys_header: str) -> Set[str]:
    p = Path(fork_keys_header)
    if not p.is_file():
        return set()
...
def main(
    ...
    try:
        ...
        # FORK: fork-specific strings live in their own ForkStrId enum
        # (lib/I18n/ForkI18nKeys.h) and are intentionally absent from the
        # upstream english.yaml. Exclude them so this validator doesn't flag
        # them as missing/unused.
        fork_keys = _load_fork_string_keys("lib/I18n/ForkI18nKeys.h")
        used_keys = used_keys - fork_keys
